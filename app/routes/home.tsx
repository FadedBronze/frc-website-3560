...
import { Lightbulb, Code, BriefcaseBusiness, Cuboid, WandSparkles, Wrench } from "lucide-react";
...
function MainText() {
  return (
    <article className="relative w-full lg:w-1/2">
      ...
      <button className="rounded-sm px-4 py-2 border-1 border-white text-xl font-[Passion_One] hover:text-[#00041A] hover:bg-white cursor-pointer">Contact Us</button>
    </article>
  )
}
          ...
          <div className="grid grid-cols-3 gap-16 pt-10 -mb-16">
            {[
              { 
                name: "PROGRAMMING",
                src: Code,
                description: "The Programming team writes the code that controls the robot, developing software for autonomous and driver-controlled functions using advanced logic and problem-solving." 
              },
              { 
                name: "MECHANICAL", 
                src: Wrench,
                description: "The Mechanical team designs, builds, and assembles the robot’s physical components, focusing on structure, movement, and durability to ensure peak performance." 
              },
              { 
                name: "ELECTRICAL", 
                src: Lightbulb,
                description: "The Electrical team handles the wiring and power systems, ensuring that all components are properly connected and the robot operates safely and efficiently." 
              },
              { 
                name: "CAD", 
                src: Cuboid,
                description: "The CAD (Computer-Aided Design) team creates detailed 3D models of the robot, using precision design to plan and simulate its structure before building." 
              },
              { 
                name: "GRAPHICS", 
                src: WandSparkles,
                description: "The Graphic team focuses on branding and visual design, creating logos, team merchandise, and digital content to represent the team’s identity." 
              },
              { 
                name: "BUSINESS", 
                src: BriefcaseBusiness,
                description: "The Business team manages outreach, fundraising, and sponsorships, ensuring the team has the resources and support needed for success. They also handle marketing and team communications." 
              }
            ].map((subteam) => (
              <article className="relative">
                <h3 className="font-[Passion_One] text-2xl mb-3">{subteam.name}</h3>
                <p className="mb-5">{subteam.description}</p>
                <button className="rounded-sm px-4 py-3 bg-blue-400 font-[Passion_One] text-[#00041A] hover:bg-white cursor-pointer text-xl">Learn More</button>
